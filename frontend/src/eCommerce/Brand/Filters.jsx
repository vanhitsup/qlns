...
import PriceFilter from "../Category/PriceFilter";

export default function Filters({ setPageConfig, name }) {
  const dispatch = useDispatch();
  const [color, setColor] = useState();
  const { list } = useSelector((state) => state.productSubCategories);
  ...
  return (
    <>
      <div>{name}</div>
      <div className='py-5'>
        <h3 className='border-b font-medium text-lg'>Category</h3>
        <Checkbox.Group
