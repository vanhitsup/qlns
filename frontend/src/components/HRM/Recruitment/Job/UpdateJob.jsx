...
const UpdateJob = () => {
  ...
  const { list: jobLocationList, loading: jobLocationLoading } = useSelector(
    (state) => state.jobLocation
  );
  const { list: jobWorkExperienceList, loading: jobWorkExperienceLoading } =
    useSelector((state) => state.jobWorkExperience);
  const { list: jobTypeList, loading: jobTypeLoading } = useSelector(
    (state) => state.jobType
  );
  const { list: jobCategoryList, loading: jobCategoryLoading } = useSelector(
    (state) => state.jobCategory
  );
  const { list: jobSkillsList, loading: jobSkillsLoading } = useSelector(
    (state) => state.jobSkills
  );
  ...
  useEffect(() => {
    ...
    form.setFieldValue(
      "jobSkillId",
      job?.jobSkills?.map((skill) => skill?.jobSkillId)
    );
    ...
    <>
      {job ? (
        <UserPrivateComponent permission={"create-job"}>
          <div className="max-w-[1000px] mx-auto" style={{ maxWidth: "100%" }}>
            <Form
              size="small"
              form={form}
              name="basic"
              ...
              <div>
                <Form.Item
                  ...
                  <Input placeholder="React.js Developer" />
                </Form.Item>
              </div>
                    ...
                    {jobLocationList &&
                      jobLocationList.map((location) => (
                        <Option key={location.id} value={location.id}>
                          {location.jobLocation}, {location.countryName}
                        </Option>
                      ))}
                  </Select>
                </Form.Item>
                ...
                <Form.Item
                  ...
                  <Select
                    ...
                    {jobWorkExperienceList &&
                      jobWorkExperienceList.map((item) => (
              ...
              <div className="flex gap-5">
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Please input Total Position!",
                    },
                  ]}>
              ...
              <div>
                <Radio.Group onChange={onChange} value={rangeValue}>
                  <Radio value={"exactSalary"}>Exact Salary</Radio>
                  <Radio value={"rangeSalary"}>Range Salary</Radio>
                </Radio.Group>
              </div>
                ...
                <div>
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Please input Exact Salary!",
                      },
                    ]}>
              ...
              <div>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Please input Job Requirement!",
                    },
                  ]}>
                  <ReactQuill
                    theme="snow"
              ...
              <Form.Item className="w-[30%] mx-auto">
                <Button
                  type="primary"
                  size="large"
                  htmlType="submit"
